...
LOCAL_TRANSLATION_DIR = Path(
    "/home/muhammed-emin-eser/desk/din/quran/eng_translation/chunked_translation/"
)
LOCAL_VIDEO_DIR = Path(
    "/home/muhammed-emin-eser/desk/din/quran/maher_workaround/Quran_cropped/"
)
LOCAL_AUDIO_DIR = Path(
    "/home/muhammed-emin-eser/desk/din/quran/maher_playlist/maher_playlist/"
)
LOCAL_OVERLAY_VIDEO_DIR = Path("/home/muhammed-emin-eser/desk/din/overlay_videos/")
...
def find_local_file(directory: Path, pattern: str) -> Path:
    ...
    try:
        ...
        raise FileNotFoundError(
            f"Keine Datei für Pattern '{pattern}' in {directory} gefunden."
        )
...
async def process_pipeline(surah_id: int, mapping_func, args):
    ...
    if run_all or args.run_mapping:
        ...
        try:
            translation_file = find_local_file(
                LOCAL_TRANSLATION_DIR, f"{surah_id}_*.txt"
            )
            local_mapping_path = find_local_file(
                BASE_DIR / "output" / "mapping", f"{translation_file.stem}.mapping"
            )
            print(
                f"[{surah_id}] Verwende existierende Mapping-Datei: {local_mapping_path}"
            )
        except FileNotFoundError as e:
            print(
                f"[{surah_id}] Fehler: Keine Mapping-Datei für die nächsten Schritte gefunden: {e}",
                file=sys.stderr,
            )
            return
    ...
    if run_all or args.run_ass:
        ...
        try:
            ...
            print(
                f"[{surah_id}] Fehler: Overlay-Video nicht gefunden: {e}",
                file=sys.stderr,
            )
            return
        ...
        not run_all and args.run_burn
    ):  # Wenn nur burn läuft, brauchen wir den ass path
        print(f"[{surah_id}] Überspringe ASS-Generierung.")
        try:
            translation_file = find_local_file(
                LOCAL_TRANSLATION_DIR, f"{surah_id}_*.txt"
            )
            ass_path = find_local_file(
                BASE_DIR / "output" / "ass", f"{translation_file.stem}.ass"
            )
            print(f"[{surah_id}] Verwende existierende ASS-Datei: {ass_path}")
        except FileNotFoundError as e:
            print(
                f"[{surah_id}] Fehler: Keine ASS-Datei für den Burn-Schritt gefunden: {e}",
                file=sys.stderr,
            )
            return
    ...
    if run_all or args.run_burn:
        ...
        if ass_path is None:
            print(
                f"[{surah_id}] Fehler: Keine ASS-Datei zum Einbrennen vorhanden.",
                file=sys.stderr,
            )
            return
        try:
        ...
        p_burn = await run_burn_subs(
            overlay_video, ass_path, LOCAL_AUDIO_DIR, final_output_dir
        )
        if p_burn.returncode != 0:
            print(
                f"[{surah_id}] Fehler beim Einbrennen der Untertitel.", file=sys.stderr
            )
            return
...
async def main():
    parser = argparse.ArgumentParser(
        description="Führt die Ayah-Aligner-Pipeline für einen Bereich von Suren aus."
    )
    ...
    parser.add_argument(
        "--run-mapping", action="store_true", help="Führt nur den Mapping-Prozess aus."
    )
    parser.add_argument(
        "--run-ass", action="store_true", help="Führt nur die ASS-Generierung aus."
    )
    parser.add_argument(
        "--run-burn",
        action="store_true",
        help="Führt nur das Einbrennen der Untertitel aus.",
    )
